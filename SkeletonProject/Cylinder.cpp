...
#include <math.h>
//=============================================================================
Cylinder::Cylinder(int height, float radius, int sideFacetsNum) : BaseObject3D((sideFacetsNum)*(height+1) + 2,(2*sideFacetsNum)*(height+1))
{
	mHeight = height;
	mRadius = radius;
...
//-----------------------------------------------------------------------------
void Cylinder::Render(IDirect3DDevice9* gd3dDevice,
...
void Cylinder::buildDemoCubeVertexBuffer(IDirect3DDevice9* gd3dDevice)
{
	...
	for (int i = 0; i <= mHeight; i++)
	{
		...
		{
			float x = mRadius*cos(currentAngle);
			float y = mRadius*sin(currentAngle);
